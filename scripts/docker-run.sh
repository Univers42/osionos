...
run_inside_container() {
  case "${COMMAND}" in
    ...
    quality) pnpm exec tsc -p packages/graph-engine/tsconfig.json --noEmit && pnpm exec tsc -p tsconfig.markdown-engine.json --noEmit && pnpm exec tsc --noEmit && pnpm exec eslint src/ packages/ --max-warnings=0 "$@" && bash scripts/check-markdown-engine-boundary.sh && exec bash scripts/check-style-tokens.sh ;;
