...
import { createPushHandler } from './bridge-push.mjs';
import { createMediaHandler } from './bridge-media.mjs';
import { createTasksHandler } from './bridge-tasks.mjs';
import { createCommentsHandler } from './bridge-comments.mjs';
import { createPublicHandler } from './bridge-public.mjs';
...
const PAGE_SURFACE_VALUES = new Set(['page', 'agent', 'home', 'folder', 'wiki', 'app', 'code']);
...
export function configFromEnv(env = process.env) {
	...
	return {
		...
		// Google OAuth for the Connections catalog `live` apps. Reuses the same
		// Google project as the Mail/Calendar bridges (GOOGLE_CLIENT_ID/SECRET);
		// the redirect URI must be registered on that project (see the migration
		// note). Empty → the /start route 503s and the UI falls back to a registry add.
		googleClientId: env.OSIONOS_GOOGLE_CLIENT_ID ?? env.GOOGLE_CLIENT_ID ?? '',
		googleClientSecret: env.OSIONOS_GOOGLE_CLIENT_SECRET ?? env.GOOGLE_CLIENT_SECRET ?? '',
		oauthRedirectUri: env.OSIONOS_OAUTH_REDIRECT_URI ?? 'https://localhost:4000/api/app-connections/oauth/callback',
		...
		// Web Push (RFC 8291/8292). Keys are generated by scripts/gen-local-env.sh
		// (or vault). Push is dormant when the keypair is unset — sends no-op.
		vapid: {
			publicKey: env.OSIONOS_VAPID_PUBLIC_KEY ?? '',
			privateKey: env.OSIONOS_VAPID_PRIVATE_KEY ?? '',
			subject: env.OSIONOS_VAPID_SUBJECT ?? 'mailto:admin@osionos.local',
		},
...
function safeUnsplashPage(value) {
	const parsed = Number(value);
...
export async function searchUnsplashPhotos({ query, perPage, page, orientation } = {}, config = configFromEnv(), fetchImpl = fetch) {
	...
	url.searchParams.set('page', String(safeUnsplashPage(page)));
...
const OPENVERSE_ASPECT = { landscape: 'wide', portrait: 'tall', squarish: 'square' };

/**
 * Keyless photo search via Openverse (CC/public-domain images, no API key) —
 * results are normalized to the Unsplash payload shape so the frontend's one
 * normalizer handles both providers.
 */
export async function searchOpenversePhotos({ query, perPage, page, orientation } = {}, fetchImpl = fetch) {
	const url = new URL('https://api.openverse.org/v1/images/');
	url.searchParams.set('q', safeUnsplashQuery(query));
	url.searchParams.set('page_size', String(safeUnsplashPerPage(perPage)));
	url.searchParams.set('page', String(safeUnsplashPage(page)));
	url.searchParams.set('aspect_ratio', OPENVERSE_ASPECT[safeUnsplashOrientation(orientation)]);
	url.searchParams.set('mature', 'false');

	const response = await fetchWithTimeout(fetchImpl, url.toString(), {
		method: 'GET',
		headers: { Accept: 'application/json', 'User-Agent': 'osionos-bridge/1.0' },
	});
	if (!response.ok) {
		throw Object.assign(new Error(`Openverse search failed with ${response.status}`), { status: 502 });
	}
	const payload = await response.json().catch(() => ({}));
	const results = (Array.isArray(payload.results) ? payload.results : []).map((item) => ({
		id: `ov-${item.id}`,
		alt_description: item.title ?? null,
		urls: { regular: item.url, small: item.thumbnail ?? item.url },
		user: { name: item.creator || 'Openverse' },
		links: { html: item.foreign_landing_url },
	})).filter((item) => typeof item.urls.regular === 'string');
	return { ok: true, results, total: Number.isFinite(payload.result_count) ? payload.result_count : undefined };
}

// Cover-search cache (per process): query key → provider payload. Successes
// only, 10-minute TTL, cleared wholesale past a cap (same pattern as the
// geocode cache below) — keeps the picker snappy and provider-rate-limit safe.
const coverSearchCache = new Map();
const COVER_SEARCH_TTL_MS = 10 * 60 * 1000;

/** Unsplash when a key is configured, Openverse otherwise — cached. */
export async function searchCoverPhotos(options = {}, config = configFromEnv(), fetchImpl = fetch) {
	const key = [safeUnsplashQuery(options.query), safeUnsplashPerPage(options.perPage),
		safeUnsplashPage(options.page), safeUnsplashOrientation(options.orientation)].join('|');
	const hit = coverSearchCache.get(key);
	if (hit && Date.now() - hit.at < COVER_SEARCH_TTL_MS) return hit.payload;

	let payload;
	if (config.unsplashAccessKey) {
		payload = await searchUnsplashPhotos(options, config, fetchImpl);
	} else {
		payload = await searchOpenversePhotos(options, fetchImpl);
	}
	if (coverSearchCache.size > 200) coverSearchCache.clear();
	coverSearchCache.set(key, { at: Date.now(), payload });
	return payload;
}
...
/** Legacy full-scan ranking — the fallback when the search_doc FTS column is absent. */
function legacyRankedRows(rows, query, limit) {
	return rows
...
/** Soft title boost for ordering FTS hits (DB already returns updated_at.desc). */
export function pageTitleScore(row, query) {
...
/** Flatten a page's JSONB block tree to plain text (snippet source). */
export function pageContentText(content) {
	const parts = [];
	const walk = (blocks) => {
		if (!Array.isArray(blocks)) return;
		for (const block of blocks) {
			if (block && typeof block.content === 'string') parts.push(block.content);
...
/** ~80-char window around the first query token (best-effort highlight source). */
export function pageSnippet(text, query) {
	const source = String(text ?? '');
	const needle = query.split(/\s+/)[0] ?? '';
	const at = needle ? source.toLowerCase().indexOf(needle) : -1;
	if (at < 0) return source.slice(0, 80).trim();
	const start = Math.max(0, at - 30);
	return (start > 0 ? '…' : '') + source.slice(start, start + 80).trim();
}

/**
 * GET /api/pages/search?workspaceId=&q=&limit= — server-side search across ALL pages
 * in the workspace. Postgres full-text (the search_doc generated tsvector, weight A
 * title + B content) via PostgREST `wfts`, so deeply nested notes are found by
 * stemmed title OR body. Owner/ACL-scoped via requireWorkspaceAccess; each entry
 * carries a content `snippet`. Falls back to a full-scan substring rank if an older
 * backend lacks the search_doc column. This is the stable contract the grobase vector
 * backend (cosine /search) will later serve unchanged.
 */
async function handlePageSearch(url, request, response, config, fetchImpl) {
	...
	if (!raw) { json(response, 200, [], config); return true; }
	try {
		const path = `osionos_pages?workspace_id=eq.${workspaceId}&archived_at=is.null`
			+ `&search_doc=wfts(english).${encodeURIComponent(raw)}`
			+ `&select=*&order=updated_at.desc&limit=${PAGE_SEARCH_MAX}`;
		const rows = await baasRest(config, fetchImpl, path);
		if (Array.isArray(rows)) {
			rows.sort((a, b) => pageTitleScore(b, query) - pageTitleScore(a, query));
			const limited = rows.slice(0, limit);
			const entries = pageRowsToEntries(limited);
			entries.forEach((entry, index) => {
				const snippet = pageSnippet(pageContentText(limited[index].content), query);
				if (snippet) entry.snippet = snippet;
			});
			json(response, 200, entries, config);
			return true;
		}
	} catch { /* search_doc absent on an older backend — fall through to the full scan */ }
	const scanned = legacyRankedRows(await listPageRows(workspaceId, config, fetchImpl, {}), query, limit);
	json(response, 200, pageRowsToEntries(scanned), config);
	return true;
}
...
const DB_SQLRO_PATH = /^\/api\/databases\/([^/]+)\/sql-ro$/;
...
/**
 * POST /api/databases/:dbId/sql-ro — run a read-only SQL query against a mount.
 * Double-gated: env OSIONOS_SQL_RO=1 here AND QUERY_ROUTER_SQL_RO on the
 * query-router (else 404 there). ACL via resolveAccessibleMount; the query-router
 * enforces the READ ONLY transaction + timeout + row cap.
 */
async function handleDatabaseSqlRo(match, request, response, config, fetchImpl) {
	if (process.env.OSIONOS_SQL_RO !== '1') throw Object.assign(new Error('Not found.'), { status: 404 });
	const dbId = requireUuid(decodeURIComponent(match[1]), 'dbId');
	await resolveAccessibleMount(request, dbId, config, fetchImpl);
	const payload = await readJson(request).catch(() => ({}));
	const sql = payload && typeof payload.sql === 'string' ? payload.sql : '';
	if (!sql.trim()) throw Object.assign(new Error('sql is required.'), { status: 422 });
	const { status, body } = await baasQueryPassthrough(config, fetchImpl, `/${dbId}/sql-ro`, { sql });
	json(response, status || 502, body ?? { rows: [] }, config);
	return true;
}
...
async function handleDatabasesPost(url, request, response, config, fetchImpl) {
	...
	const sqlRoMatch = DB_SQLRO_PATH.exec(url.pathname);
	if (sqlRoMatch) return handleDatabaseSqlRo(sqlRoMatch, request, response, config, fetchImpl);
...
// ── Favorites (per-user starred pages) ──────────────────────────────────────
export function favoritePageIdFromPath(pathname) {
	const match = /^\/api\/favorites\/([^/]+)$/.exec(pathname);
	if (!match) return '';
	return requireUuid(decodeURIComponent(match[1]), 'pageId');
}

/** GET /api/favorites — the session user's starred page ids (most-recent first). */
async function handleFavoritesList(request, response, config, fetchImpl) {
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const query = postgrestQuery({ user_id: `eq.${authContext.userId}`, select: 'page_id', order: 'created_at.desc' });
	const rows = await baasRest(config, fetchImpl, `osionos_page_favorites?${query}`);
	json(response, 200, { ok: true, pageIds: (Array.isArray(rows) ? rows : []).map((row) => row.page_id) }, config);
	return true;
}

/** POST /api/favorites {pageId} — star a page the caller can read (idempotent). */
async function handleFavoriteAdd(request, response, config, fetchImpl) {
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const payload = await readJson(request, PAGE_JSON_BODY_LIMIT_BYTES);
	const pageId = requireUuid(payload.pageId, 'pageId');
	const row = await fetchPageRow(pageId, config, fetchImpl);
	if (!row) throw Object.assign(new Error('Page not found.'), { status: 404 });
	await ownerOrWorkspaceAccess(request, row, 'read', config, fetchImpl);
	await baasRest(config, fetchImpl, 'osionos_page_favorites', {
		method: 'POST',
		body: { user_id: authContext.userId, page_id: pageId },
		prefer: 'resolution=merge-duplicates,return=minimal',
	});
	json(response, 200, { ok: true, pageId }, config);
	return true;
}

/** DELETE /api/favorites/:pageId — unstar (idempotent). */
async function handleFavoriteDelete(url, request, response, config, fetchImpl) {
	const pageId = favoritePageIdFromPath(url.pathname);
	if (!pageId) return false;
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	await baasRest(config, fetchImpl,
		`osionos_page_favorites?user_id=eq.${authContext.userId}&page_id=eq.${pageId}`,
		{ method: 'DELETE', prefer: 'return=minimal' });
	json(response, 200, { ok: true, pageId }, config);
	return true;
}

function backlinkRefsFromRows(rows) {
	return (Array.isArray(rows) ? rows : []).map((row) => ({ id: row.id, title: row.title || 'Untitled', icon: row.icon ?? null }));
}

/**
 * GET /api/pages/:id/backlinks — inline [[page:]] backlinks + unlinked mentions.
 * `linked` = pages (in a workspace the caller can access) that link to :id.
 * `mentions` = pages whose title/body match :id's title (FTS) but don't link it.
 */
async function handlePageBacklinks(url, request, response, config, fetchImpl) {
	const pageId = pageSubresourceIdFromPath(url.pathname, 'backlinks');
	if (!pageId) return false;
	const row = await fetchPageRow(pageId, config, fetchImpl);
	if (!row) throw Object.assign(new Error('Page not found.'), { status: 404 });
	const authContext = await ownerOrWorkspaceAccess(request, row, 'read', config, fetchImpl);
	const accessible = [...new Set([...authContext.workspaceIds, ...await memberWorkspaceIds(authContext.userId, config, fetchImpl)])].filter((id) => UUID_REGEX.test(id));
	if (accessible.length === 0) { json(response, 200, { ok: true, linked: [], mentions: [] }, config); return true; }
	const inWorkspace = `workspace_id=in.(${accessible.join(',')})`;
	const linkRows = await baasRest(config, fetchImpl, `osionos_page_links?target_page_id=eq.${pageId}&${inWorkspace}&select=source_page_id`);
	const sourceIds = [...new Set((Array.isArray(linkRows) ? linkRows : []).map((r) => r.source_page_id).filter((id) => id && id !== pageId))];
	const linked = sourceIds.length
		? backlinkRefsFromRows(await baasRest(config, fetchImpl, `osionos_pages?id=in.(${sourceIds.join(',')})&archived_at=is.null&select=id,title,icon`))
		: [];
	const title = String(row.title || '').trim();
	let mentions = [];
	if (title) {
		const excluded = new Set([pageId, ...sourceIds]);
		const hits = await baasRest(config, fetchImpl, `osionos_pages?${inWorkspace}&archived_at=is.null&search_doc=wfts(english).${encodeURIComponent(`"${title}"`)}&select=id,title,icon&limit=20`).catch(() => []);
		mentions = backlinkRefsFromRows((Array.isArray(hits) ? hits : []).filter((hit) => !excluded.has(hit.id)));
	}
	json(response, 200, { ok: true, linked, mentions }, config);
	return true;
}

async function handlePagesGet(url, request, response, config, fetchImpl) {
	if (url.pathname === '/api/favorites') return handleFavoritesList(request, response, config, fetchImpl);
	if (await handlePageBacklinks(url, request, response, config, fetchImpl)) return true;
	...
	if (await handleAppConnectionsGet(url, request, response, config, fetchImpl)) return true;
	if (await handleAppConnectionsOAuthCallback(url, request, response, config, fetchImpl)) return true;
...
// ── App connections (Settings → Connections catalog) ────────────────────────
// One row per (owner, provider) in osionos_app_connections. owner_id is stamped
// from the verified app-session token, never trusted from the body — mirrors the
// object-databases handlers above. The frontend sends the PAGE app-session JWT
// (getActivePageJwt), the same `osionos_v1.` token these routes verify; the old
// tab used getActiveJwt(), which blanks for a bridge session, so it silently
// never reached the bridge (and also hit `/api/connections`, the social route).
const APP_CONNECTION_SELECT = 'id,owner_id,provider,label,scopes,status,external_account,connected_at,last_sync_at,created_at,updated_at,removed_at';
...
async function handleAppConnectionsGet(url, request, response, config, fetchImpl) {
	if (url.pathname !== '/api/app-connections') return false;
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const rows = await baasRest(config, fetchImpl,
		`osionos_app_connections?${postgrestQuery({ owner_id: `eq.${authContext.userId}`, removed_at: 'is.null', select: APP_CONNECTION_SELECT, order: 'connected_at.desc' })}`);
	const connections = (Array.isArray(rows) ? rows : []).map(mapConnectionRow).filter(Boolean);
	json(response, 200, { connections }, config);
	return true;
}

// Owner-scoped upsert on (owner_id, provider); revives a previously-removed row
// (removed_at → null). Shared by the manual "Add" (below) and the Google OAuth
// callback so both write an identical shape.
async function upsertAppConnection(config, fetchImpl, { ownerId, provider, label, scopes, status, externalAccount }) {
	const now = new Date().toISOString();
	const rows = await baasRest(config, fetchImpl, 'osionos_app_connections?on_conflict=owner_id,provider', {
		method: 'POST',
		body: {
			owner_id: ownerId,
			provider,
			label: label || provider,
			scopes: Array.isArray(scopes) ? scopes.slice(0, 40) : [],
...
async function handleAppConnectionsUpsert(url, request, response, config, fetchImpl) {
	if (url.pathname !== '/api/app-connections') return false;
	const payload = await readJson(request);
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const provider = safeText(payload.provider, 80);
	if (!provider) throw Object.assign(new Error('provider is required.'), { status: 422 });
	const scopes = Array.isArray(payload.scopes)
		? payload.scopes.map((scope) => safeText(scope, 200)).filter(Boolean).slice(0, 40)
		: [];
	const row = await upsertAppConnection(config, fetchImpl, {
		ownerId: authContext.userId,
		provider,
		label: safeText(payload.label, 200),
		scopes,
		status: safeText(payload.status, 40),
		externalAccount: payload.externalAccount ? safeText(payload.externalAccount, 200) : null,
	});
	json(response, 200, { connection: mapConnectionRow(row) }, config);
	return true;
}

async function handleAppConnectionsDelete(url, request, response, config, fetchImpl) {
	const match = /^\/api\/app-connections\/([^/]+)$/.exec(url.pathname);
	if (!match) return false;
	const id = match[1];
	if (!UUID_REGEX.test(id)) throw Object.assign(new Error('connection id is invalid.'), { status: 422 });
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	await baasRest(config, fetchImpl,
		`osionos_app_connections?${postgrestQuery({ id: `eq.${id}`, owner_id: `eq.${authContext.userId}` })}`,
		{ method: 'DELETE', prefer: 'return=minimal' });
	json(response, 200, { ok: true, id }, config);
	return true;
}

// ── Google OAuth for the Connections catalog `live` apps ─────────────────────
// Ported from the Mail bridge (apps/mail/bridge/server.mjs): real Google consent
// + code exchange. The connection row keeps only the VERIFIED account email +
// granted scopes, NOT the refresh token — nothing in osionos consumes a
// connections-table Google token (Gmail/Calendar data have their own dedicated
// bridges), so persisting a live secret with no reader would be pure liability.
// ponytail: identity-only connection; add encrypted-token storage the day a
// feature actually calls Google through this row.
//
// State is a self-contained HMAC token (no server-side store): it binds owner +
// app + return URL + a short expiry, signed with the app-session secret. The
// browser callback carries no Authorization header, so this signed state is the
// ONLY anchor tying the code back to the account that started the flow.
const GOOGLE_OAUTH_APPS = {
	'google-calendar': { label: 'Google Calendar', scope: 'https://www.googleapis.com/auth/calendar' },
	'gmail': { label: 'Gmail', scope: 'https://www.googleapis.com/auth/gmail.modify' },
	'google-drive': { label: 'Google Drive', scope: 'https://www.googleapis.com/auth/drive.readonly' },
	'google-contacts': { label: 'Google Contacts', scope: 'https://www.googleapis.com/auth/contacts.readonly' },
};
const OAUTH_STATE_TTL_SECONDS = 600;

export function signOAuthState(payload, config) {
	if (!config.appSessionSecret) throw Object.assign(new Error('osionos app session secret is not configured.'), { status: 503 });
	const body = Buffer.from(JSON.stringify(payload)).toString('base64url');
	const sig = createHmac('sha256', config.appSessionSecret).update(body).digest('base64url');
	return `${body}.${sig}`;
}

export function verifyOAuthState(state, config, now = Date.now()) {
	if (!config.appSessionSecret) throw Object.assign(new Error('osionos app session secret is not configured.'), { status: 503 });
	const [body, sig, extra] = safeText(state, 4096).split('.');
	if (!body || !sig || extra !== undefined) throw Object.assign(new Error('OAuth state is invalid.'), { status: 400 });
	const expected = createHmac('sha256', config.appSessionSecret).update(body).digest('base64url');
	if (!safeCompareText(sig, expected)) throw Object.assign(new Error('OAuth state signature is invalid.'), { status: 400 });
	let payload;
	try { payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')); }
	catch { throw Object.assign(new Error('OAuth state payload is invalid.'), { status: 400 }); }
	if (!payload || typeof payload !== 'object' || !UUID_REGEX.test(String(payload.sub ?? '')) || !GOOGLE_OAUTH_APPS[payload.app]) {
		throw Object.assign(new Error('OAuth state payload is invalid.'), { status: 400 });
	}
	if (!Number.isFinite(Number(payload.exp)) || Number(payload.exp) <= Math.floor(now / 1000)) {
		throw Object.assign(new Error('OAuth state has expired.'), { status: 400 });
	}
	return payload;
}

function oauthSameOriginReturnTo(returnTo, config) {
	try {
		const parsed = new URL(safeText(returnTo, 2048));
		if (parsed.origin === new URL(config.appUrl).origin) return parsed.toString();
	} catch { /* fall through to the app root */ }
	return config.appUrl;
}

function oauthRedirect(response, config, returnTo, { error, connected } = {}) {
	const target = new URL(oauthSameOriginReturnTo(returnTo, config));
	if (error) target.searchParams.set('connection_error', error);
	else if (connected) target.searchParams.set('connected', connected);
	response.writeHead(302, { Location: target.toString() });
	response.end();
}

async function handleAppConnectionsOAuthStart(url, request, response, config, fetchImpl) {
	const match = /^\/api\/app-connections\/oauth\/([^/]+)\/start$/.exec(url.pathname);
	if (!match) return false;
	const app = GOOGLE_OAUTH_APPS[match[1]];
	if (!app) throw Object.assign(new Error('Unknown OAuth app.'), { status: 404 });
	if (!config.googleClientId || !config.googleClientSecret) {
		throw Object.assign(new Error('Google OAuth is not configured on the bridge.'), { status: 503 });
	}
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const payload = await readJson(request).catch(() => ({}));
	const returnTo = oauthSameOriginReturnTo(payload.returnTo, config);
	const state = signOAuthState({ sub: authContext.userId, app: match[1], rt: returnTo, exp: Math.floor(Date.now() / 1000) + OAUTH_STATE_TTL_SECONDS }, config);
	const params = new URLSearchParams({
		client_id: config.googleClientId,
		redirect_uri: config.oauthRedirectUri,
		response_type: 'code',
		scope: `openid email ${app.scope}`,
		access_type: 'offline',
		prompt: 'consent',
		include_granted_scopes: 'true',
		state,
	});
	json(response, 200, { authorizeUrl: `https://accounts.google.com/o/oauth2/v2/auth?${params}` }, config);
	return true;
}

async function exchangeGoogleCode(code, config, fetchImpl) {
...
async function handleAppConnectionsOAuthCallback(url, request, response, config, fetchImpl) {
	if (url.pathname !== '/api/app-connections/oauth/callback') return false;
	const googleError = url.searchParams.get('error');
	if (googleError) { oauthRedirect(response, config, config.appUrl, { error: `google_${safeText(googleError, 80)}` }); return true; }
	let state;
	try { state = verifyOAuthState(url.searchParams.get('state'), config); }
	catch { oauthRedirect(response, config, config.appUrl, { error: 'invalid_state' }); return true; }
	const code = url.searchParams.get('code');
	if (!code) { oauthRedirect(response, config, state.rt, { error: 'missing_code' }); return true; }
	try {
		const tokens = await exchangeGoogleCode(code, config, fetchImpl);
		await upsertAppConnection(config, fetchImpl, {
			ownerId: state.sub,
			provider: state.app,
			label: GOOGLE_OAUTH_APPS[state.app].label,
			scopes: [GOOGLE_OAUTH_APPS[state.app].scope],
			status: 'connected',
			externalAccount: emailFromIdToken(tokens.id_token),
		});
		oauthRedirect(response, config, state.rt, { connected: state.app });
	} catch {
		oauthRedirect(response, config, state.rt, { error: 'exchange_failed' });
	}
	return true;
}
...
/**
 * POST /api/workspaces {name, slug} — create a workspace owned by the caller.
 * Inserts osionos_workspaces + the owner osionos_workspace_members row (which is
 * what grants access — no new token needed). The slug is made unique server-side.
 */
async function handleWorkspaceCreate(request, response, config, fetchImpl) {
	const authContext = verifyAppSessionToken(bearerToken(request), config);
	const payload = await readJson(request, PAGE_JSON_BODY_LIMIT_BYTES);
	const name = (safeText(payload.name, 120) || '').trim() || 'New workspace';
	const baseSlug = (safeText(payload.slug, 80) || '').trim().toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || 'workspace';
	const id = randomUUID();
	const slug = `${baseSlug}-${randomUUID().slice(0, 8)}`;
	const ownerPermissions = ['create', 'read', 'update', 'delete', 'admin'];
	const rows = await baasRest(config, fetchImpl, 'osionos_workspaces', {
		method: 'POST',
		body: { id, owner_id: authContext.userId, name, slug, source: 'bridge' },
		prefer: 'return=representation',
	});
	const row = Array.isArray(rows) && rows[0] ? rows[0] : { id, owner_id: authContext.userId, name, slug };
	await baasRest(config, fetchImpl, 'osionos_workspace_members?on_conflict=workspace_id,user_id', {
		method: 'POST',
		body: { workspace_id: id, user_id: authContext.userId, role: 'owner', permissions: ownerPermissions },
		prefer: 'resolution=ignore-duplicates,return=minimal',
	});
	json(response, 200, workspaceRowToEntry(row, { role: 'owner', permissions: ownerPermissions }), config);
	return true;
}

async function handlePagesPost(url, request, response, config, fetchImpl) {
	if (url.pathname === '/api/workspaces') return handleWorkspaceCreate(request, response, config, fetchImpl);
	...
	if (await handleAppConnectionsUpsert(url, request, response, config, fetchImpl)) return true;
	if (await handleAppConnectionsOAuthStart(url, request, response, config, fetchImpl)) return true;
	...
	if (url.pathname === '/api/favorites') return handleFavoriteAdd(request, response, config, fetchImpl);
...
async function handleBridgeGet(url, response, config, fetchImpl) {
	...
	json(response, 200, await searchCoverPhotos({
...
async function handleBridgeRequest(request, response, context) {
	...
	if (await context.social.push(url, request, response, context.config)) return;
	if (await context.social.media(url, request, response, context.config)) return;
	if (await context.social.tasks(url, request, response, context.config)) return;
	if (await context.social.comments(url, request, response, context.config)) return;
	if (await context.social.publicPages(url, request, response, context.config)) return;
	...
	if (request.method === 'DELETE' && await handleFavoriteDelete(url, request, response, context.config, context.fetchImpl)) return;
	if (request.method === 'DELETE' && await handleObjectDatabasesDelete(url, request, response, context.config, context.fetchImpl)) return;
	if (request.method === 'DELETE' && await handleAppConnectionsDelete(url, request, response, context.config, context.fetchImpl)) return;
...
export function createBridgeServer(options = {}) {
	...
	const social = options.social ?? {
		...
		push: createPushHandler({ config, verifySession: verifyAppSessionToken, fetchImpl }),
		media: createMediaHandler({ config, verifySession: verifyAppSessionToken, fetchImpl }),
			tasks: createTasksHandler({ config, verifySession: verifyAppSessionToken, fetchImpl }),
			comments: createCommentsHandler({
				config, verifySession: verifyAppSessionToken, fetchImpl,
				verifyPageAccess: async (request, pageId, permission) => {
					const row = await fetchPageRow(pageId, config, fetchImpl);
					if (!row) throw Object.assign(new Error('Page not found.'), { status: 404 });
					const authContext = await ownerOrWorkspaceAccess(request, row, permission, config, fetchImpl);
			...
			publicPages: createPublicHandler({
				config, verifySession: verifyAppSessionToken, fetchImpl,
				verifyPageAccess: async (request, pageId, permission) => {
					const row = await fetchPageRow(pageId, config, fetchImpl);
					if (!row) throw Object.assign(new Error('Page not found.'), { status: 404 });
