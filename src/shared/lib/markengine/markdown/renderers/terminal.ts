  ...
  switch (node.type) {
    ...
    case "heading": {
      ...
      if (node.level <= 2) {
        ...
        const underline = underChar.repeat(
          Math.min(
            stripAnsi(text).length + marker.length + 1,
            ctx.o.width - prefix.length,
          ),
        );
    ...
    case "blockquote": {
      ...
      const inner = node.children
        .map((ch) => renderBlock(ch, innerCtx))
        .join("");
      ...
      return (
        inner
          ...
          .map((line) =>
            line
              ? `${prefix}${bar} ${c(ctx, C.quote)}${line.trimStart()}${reset(ctx)}`
              : "",
          )
          .join("\n") + "\n"
      );
    ...
    case "callout": {
      ...
      const bodyLines = body
        .split("\n")
        .map((line) => (line ? `${prefix}${bar} ${line.trimStart()}` : ""))
        .join("\n");
      return `${header}\n${bodyLines}\n`;
    }
        ...
        node.items
          .map((item) => {
            ...
            const defs = item.definitions
              .map((def) => `${prefix}  ${renderInlines(def, ctx)}`)
              .join("\n");
            return `${term}\n${defs}`;
          })
          .join("\n") + "\n"
