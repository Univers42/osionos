...
export function AddAxisButton({ axis, onAdd }: Readonly<{ axis: Axis; onAdd: (axis: Axis, count: number) => void }>) {
  ...
  return (
    <div className={`group/add ${anchorClass} z-[var(--osio-z-raised)] flex items-center gap-1`} data-table-handle>
      ...
      {dragCount > 0 && <span className="pointer-events-none rounded-full bg-[var(--osio-accent)] px-1.5 text-[11px] font-semibold leading-5 text-[var(--osio-accent-fg,#fff)] shadow">+{dragCount}</span>}
      <div className={`hidden group-hover/add:flex ${isColumn ? "flex-col" : "flex-row"} items-center gap-0.5 rounded-full border border-[var(--osio-border-default)] bg-[var(--osio-bg-surface)] px-1 py-0.5 shadow-sm`}>
        {[2, 3, 5].map((count) => (
          <button key={count} type="button" title={`Add ${count} ${label}s`} onClick={() => onAdd(axis, count)} className="rounded px-1.5 text-[11px] font-semibold leading-5 text-[var(--osio-fg-muted)] hover:text-[var(--osio-fg-default)]">
            {count}
          </button>
        ))}
      </div>
