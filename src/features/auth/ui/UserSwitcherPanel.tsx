...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ tab, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[var(--color-backdrop)] px-4">
      <div className="flex h-[min(720px,90vh)] w-full max-w-4xl overflow-hidden rounded-xl border border-[var(--color-line)] bg-[var(--color-surface-primary)] shadow-2xl">
        <aside className="w-60 shrink-0 border-r border-[var(--color-line)] bg-[var(--color-surface-secondary)] p-3">
          <p className="px-2 py-1 text-xs font-medium text-[var(--color-ink-faint)]">Account</p>
          <button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-[var(--color-surface-hover)]">
            <AssetRenderer value={persona?.emoji ?? '👤'} size={20} />
            {persona?.name ?? 'Profile'}
          </button>
          <p className="mt-4 px-2 py-1 text-xs font-medium text-[var(--color-ink-faint)]">Workspace</p>
          <button
            onClick={() => setActiveTab('settings')}
            className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm ${activeTab === 'settings' ? 'bg-[var(--color-surface-tertiary)]' : 'hover:bg-[var(--color-surface-hover)]'}`}
          >
            <Settings size={16} /> General
          </button>
          <button
            onClick={() => setActiveTab('people')}
            className={`flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm ${activeTab === 'people' ? 'bg-[var(--color-surface-tertiary)]' : 'hover:bg-[var(--color-surface-hover)]'}`}
          >
            <Users size={16} /> People
          </button>
        </aside>
        <section className="relative flex-1 overflow-auto p-8">
          <button
            type="button"
            onClick={onClose}
            className="absolute right-3 top-3 rounded-full p-1 hover:bg-[var(--color-surface-hover)]"
          ...
          {activeTab === 'settings' ? (
            <div className="max-w-2xl">
              ...
              <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
                Configuration générale du workspace actif.
              </p>
              <div className="mt-8 border-t border-[var(--color-line)] pt-4">
                ...
                <div className="mt-4 grid gap-4 text-sm">
                  <div className="flex justify-between border-b border-[var(--color-line)] pb-3">
                    <span className="text-[var(--color-ink-muted)]">Nom</span>
                    <span>{session?.privateWorkspaces[0]?.name ?? 'Workspace'}</span>
                  </div>
                  <div className="flex justify-between border-b border-[var(--color-line)] pb-3">
                    <span className="text-[var(--color-ink-muted)]">Membres</span>
            ...
            <div className="max-w-2xl">
              ...
              <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
                Comptes actifs et membres disponibles dans le playground.
              </p>
              <div className="mt-6 divide-y divide-[var(--color-line)] rounded-lg border border-[var(--color-line)]">
                {members.map((member) => (
                  <div key={member.email} className="flex items-center gap-3 p-3">
                    <AssetRenderer value={member.emoji} size={24} />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium">{member.name}</p>
                      <p className="truncate text-xs text-[var(--color-ink-muted)]">{member.email}</p>
                    </div>
                    <span className="text-xs text-[var(--color-ink-muted)]">{member.roleBadge}</span>
                  </div>
                ))}
              </div>
            </div>
