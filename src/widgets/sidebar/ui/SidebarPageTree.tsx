...
export const SidebarPageTree: React.FC<SidebarPageTreeProps> = ({
  ...
  const handleAddChildToRecent = async (e: React.MouseEvent, r: ActivePage) => {
    e.stopPropagation();
    const child = await addPage(r.workspaceId, 'Untitled', jwt, r.id);
    if (child) {
      openPage({ id: child._id, workspaceId: r.workspaceId, kind: 'page', title: child.title });
    }
  };
