...
export const TabButton: React.FC<Props> = ({ tab, paneId, active, onSelect, onClose }) => {
  ...
  return (
    <div
      ...
      className={[
        "group/tab flex items-center gap-1 h-8 pl-2 pr-1 max-w-[180px] shrink-0 cursor-pointer select-none",
        "border-r border-[var(--osio-border)] text-xs",
        active
          ? "bg-[var(--osio-bg-page)] text-[var(--osio-fg-default)]"
          : "bg-[var(--osio-bg-muted)] text-[var(--osio-fg-muted)] hover:bg-[var(--osio-bg-hover)]",
      ].join(" ")}
    >
      {tab.kind === "folder"
        ? <Folder size={13} className="shrink-0 text-[var(--osio-fg-muted)]" />
        : <IconValueView value={tab.icon ?? "icon:page"} size={13} className="shrink-0" />}
      <span className="truncate flex-1">{tab.title || "Untitled"}</span>
      <button
        type="button"
        className="shrink-0 rounded p-0.5 opacity-0 group-hover/tab:opacity-100 hover:bg-[var(--osio-bg-hover)]"
